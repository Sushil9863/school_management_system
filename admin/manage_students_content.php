...
if ($class_id_filter) {
    $stmt = $conn->prepare($query . " WHERE c.id = ? ORDER BY s.id DESC");
    $stmt->bind_param("i", $class_id_filter);
    $stmt->execute();
    $students = $stmt->get_result();
} else {
    $students = $conn->query($query . " ORDER BY s.id DESC");
}
...
if (isset($_GET['export']) && $_GET['export'] === 'pdf') {
    ...
    foreach ($students as $row) {
        echo "<tr>
                ...
                <td>{$row['gender']}</td>
                <td>{$row['dob']}</td>
                <td>Grade {$row['class_grade']} - {$row['class_section']}</td>
    ...
    // Clear buffer before sending PDF
...
<body class="bg-gradient-to-br from-blue-200 to-purple-300 min-h-screen p-6">
  <div class="max-w-6xl mx-auto bg-white/60 backdrop-blur-md p-6 rounded-xl shadow-xl">
    <div class="flex justify-between items-center mb-6">
      ...
      <div>
        ...
        <button onclick="openAddModal()" class="ml-2 bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">+ Add Student</button>
      </div>
    </div>
    ...
    <form method="GET" class="mb-4 flex items-center gap-3">
      ...
      <input type="text" id="searchInput" onkeyup="filterTable()" placeholder="Search..." class="px-4 py-2 border rounded flex-1">
    </form>
      ...
      <tbody>
        <?php while ($row = $students->fetch_assoc()): ?>
          <tr class="border-b hover:bg-gray-100">
            ...
            <td class="py-2 px-4">Grade <?= htmlspecialchars($row['class_grade']) ?> - <?= htmlspecialchars($row['class_section']) ?></td>
            <td class="py-2 px-4"><?= htmlspecialchars($row['parent_name']) ?></td>
            <td class="py-2 px-4 text-center space-x-2">
              <button onclick='openEditModal(<?= json_encode($row) ?>)' class="bg-yellow-400 text-white px-3 py-1 rounded hover:bg-yellow-500">Edit</button>
              <button onclick='openDeleteModal(<?= $row['id'] ?>)' class="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600">Delete</button>
            </td>
          </tr>
        <?php endwhile; ?>
      </tbody>
  ...
  <!-- Modals -->
  <div id="studentModal" onclick="closeModal(event)" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden z-50 flex items-center justify-center">
    <div onclick="event.stopPropagation();" class="bg-white/30 backdrop-blur-lg shadow-2xl hover:ring-4 hover:ring-blue-400 rounded-2xl p-6 w-full max-w-2xl animate-fade-in">
      ...
      <form method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4 text-white">
        ...
        <div><label>Full Name</label><input type="text" name="full_name" id="full_name" required class="w-full px-3 py-2 rounded bg-white/80 text-black"></div>
        <div>
          <label>Gender</label>
        ...
        <div><label>DOB</label><input type="date" name="dob" id="dob" required class="w-full px-3 py-2 rounded bg-white/80 text-black"></div>
        <div>
          <label>Class</label>
  ...
  <div id="deleteModal" onclick="closeModal(event)" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden z-50 flex items-center justify-center">
    <div onclick="event.stopPropagation();" class="bg-white/30 backdrop-blur-lg p-6 rounded-2xl shadow-2xl hover:ring-4 hover:ring-red-400 w-full max-w-md animate-fade-in text-white text-center">
      <form method="POST">
        ...
        <div class="flex justify-center space-x-4">
          <button type="button" onclick="closeAllModals()" class="bg-gray-300 text-black px-4 py-2 rounded">Cancel</button>
          <button type="submit" class="bg-gradient-to-r from-red-600 to-pink-600 text-white px-6 py-2 rounded hover:scale-105">Delete</button>
        </div>
      </form>
    </div>
  </div>
