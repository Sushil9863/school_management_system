...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'edit') {
    ...
    $stmt->bind_param("ssssii", $_POST['full_name'], $_POST['address'], $_POST['phone'], $_POST['email'], $teacher_id, $school_id);
    $stmt->execute();
...
<body class="bg-gradient-to-r from-blue-100 to-purple-100 min-h-screen p-6">
  <div class="max-w-6xl mx-auto bg-white rounded-lg shadow p-6">
    <div class="flex justify-between items-center mb-4">
      ...
      <div>
        <a href="?export=pdf" target="_blank" class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700">📄 Export
          PDF</a>
        <button onclick="openAddModal()" class="ml-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded">+ Add
          Teacher</button>
      </div>
    </div>

    <div class="mb-4">
      <input type="text" id="searchInput" onkeyup="filterTable('searchInput','teacherTable')"
        placeholder="Search by name..." class="w-full md:w-64 px-4 py-2 rounded border border-gray-300 shadow-sm">
    </div>
  ...
  <!-- Add/Edit Modal -->
  <div id="teacherModal" onclick="closeModal(event)"
      ...
      <form method="POST" class="space-y-4" id="teacherForm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <div id="full_name_error" class="error-message">Full name must be 3-50 characters and contain only letters
              and spaces</div>
          </div>
          <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4" id="authFields">
          ...
          <div>
            ...
            <div id="password_error" class="error-message">Password must be 8-20 characters with at least one uppercase,
              one lowercase, and one number</div>
          </div>
        </div>
        ...
        <div class="flex justify-end space-x-3 mt-4">
          ...
          <button type="submit" id="submitBtn"
            class="px-6 py-2 rounded bg-gradient-to-r from-blue-600 to-indigo-600 text-white hover:scale-105 hover:shadow-xl font-semibold">Save</button>
        </div>
      </form>
    </div>
  </div>
